...
import {
  Plus, X, Trash2, MessageSquare, TriangleAlert, Link2, Target, Package,
  SlidersHorizontal,
} from "lucide-react";
import {
  useCreateEntity, useUpdateEntity, useDeleteEntity,
} from "@/lib/api/hooks";
import { resolveDep, wouldConflict, initials, type ResolvedDep } from "@/lib/tasks";
...
export function CardModal({
  ...
  const [form, setForm] = useState(() => ({
    ...
    category: task?.category ?? defaultCategoryId ?? ws.categories[0]?.id ?? "none",
    phase: task?.phase ?? "none",
    assignees: (task?.assignees ?? []).join(", "),
    start: task?.start ?? "",
    end: task?.end ?? "",
    tags: task?.tags ?? ([] as string[]),
    deps: task?.deps ?? ([] as TaskDep[]),
  }));
  const set = (k: string, v: unknown) => setForm((f) => ({ ...f, [k]: v }));

  const [showPhase, setShowPhase] = useState(!!task?.phase);
  const [showTags, setShowTags] = useState((task?.tags?.length ?? 0) > 0);
  ...
  function save() {
    ...
    const payload = {
      ...
      start: form.start, end: form.end, tags: form.tags, deps: form.deps,
      comments: task?.comments ?? [], custom: task?.custom ?? {},
    };
  ...
  function remove() {
    ...
    del.mutate(task.id, { onError: (e) => toast.error((e as Error).message) });
    onOpenChange(false);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
        <DialogHeader><DialogTitle className="font-serif-display font-medium">{task ? "Edit task" : "New task"}</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label>Title</Label>
          ...
          <div className="border-t pt-4">
            <DepEditor task={task} ws={ws} projectId={projectId} deps={form.deps} onChange={(next) => set("deps", next)} />
          </div>

          {task && (
            <div className="border-t pt-4">
              <RiskLinker task={task} ws={ws} projectId={projectId} />
            </div>
          )}

          <div className="flex flex-wrap items-center gap-2 border-t pt-3">
            <span className="text-muted-foreground flex items-center gap-1 text-xs">
              <SlidersHorizontal className="size-3" /> Optional:
            </span>
            {ws.phases.length > 0 && (
              <button type="button" onClick={() => setShowPhase((v) => !v)}
                className={cn("rounded-full border px-2.5 py-0.5 text-xs transition", showPhase ? "bg-primary/10 border-primary/30 text-primary" : "text-muted-foreground hover:bg-muted")}>
                Phase
              </button>
            )}
            {ws.tags.length > 0 && (
              <button type="button" onClick={() => setShowTags((v) => !v)}
                className={cn("rounded-full border px-2.5 py-0.5 text-xs transition", showTags ? "bg-primary/10 border-primary/30 text-primary" : "text-muted-foreground hover:bg-muted")}>
                Tags
              </button>
            )}
          </div>

          {showPhase && ws.phases.length > 0 && (
            <div className="space-y-1.5">
              <Label>Phase</Label>
              <Select value={form.phase} onValueChange={(v) => set("phase", v)}>
                ...
                <SelectContent>
                  ...
                  {ws.phases.map((p) => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}

          {showTags && ws.tags.length > 0 && (
            <div className="space-y-1.5">
              <Label>Tags</Label>
              <div className="flex flex-wrap gap-1.5">
                {ws.tags.map((t) => (
                  <button key={t.id} type="button"
                    onClick={() => set("tags", form.tags.includes(t.id) ? form.tags.filter((x: string) => x !== t.id) : [...form.tags, t.id])}
                    className={cn("rounded px-2 py-0.5 text-xs transition", form.tags.includes(t.id) ? accent(t.color).soft : "bg-muted text-muted-foreground")}>
                    {t.label}
                  </button>
                ))}
              </div>
          ...
          {/* custom fields */}
          {task && Object.keys(task.custom ?? {}).length > 0 && (
            <div className="space-y-1.5 border-t pt-4">
              <Label>Custom fields</Label>
              <div className="space-y-1">
                {Object.entries(task.custom as Record<string, unknown>).map(([k, v]) => (
            ...
            <div className="border-t pt-4">
              <h4 className="mb-3 flex items-center gap-1.5 text-sm font-medium">
                ...
                {(task.comments?.length ?? 0) > 0 && (
                  <span className="bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 text-xs">{task.comments.length}</span>
                )}
              </h4>
              {(task.comments ?? []).length === 0 && <p className="text-muted-foreground mb-3 text-xs">No comments yet.</p>}
              {(task.comments ?? []).map((c) => (
